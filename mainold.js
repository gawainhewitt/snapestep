...
let seqPropertiesObject = {};
for(let i = 0; i < seqRows; i++){
  seqPropertiesObject[`row${i}`] = new Array ();
}

let seqStepObject = {};
for(let i = 0; i < seqRows; i++){
  seqStepObject[`row${i}`] = new Array ();
}
...
function createButtonPositions() {
  ...
  for(let i = 0; i < seqRows; i++){
    for(let j = 0; j < seqSteps; j++){
      seqPropertiesObject[`row${i}`].push({
        x: seqStepDistance,
        y: seqRowPosition,
        image: seqOff
      });
...
function drawSynth(step) { // instead of using the draw function at 60 frames a second we will call this function when something changes
  ...
  if(save.status){
    ...
    for(let i = 0; i < seqRows; i++){
      for(let j = 0; j < seqSteps; j++){
        if((j === step) && (seqStepObject[`row${i}`][j] === 0)){ // if this is the current step and the step is "off"
          image(seqStep1, seqPropertiesObject[`row${i}`][j].x, seqPropertiesObject[`row${i}`][j].y, seqWidth, seqHeight); // then yellow seq for this step
        }else if((j === step) && (seqStepObject[`row${i}`][j] === 1)){ // if this is the current step and the step is "on"
          image(seqStep2, seqPropertiesObject[`row${i}`][j].x, seqPropertiesObject[`row${i}`][j].y, seqWidth, seqHeight); // then purple seq for this step
        }
        else{
          image(seqPropertiesObject[`row${i}`][j].image, seqPropertiesObject[`row${i}`][j].x, seqPropertiesObject[`row${i}`][j].y, seqWidth, seqHeight); // otherwise seq colour reflects step state
        }
      }
    }
  ...
  // myJSON = Flatted.stringify(seqPropertiesObject);
  // console.log(myJSON);
}
...
function handleClick(e){
  if(soundOn) {
    if(save.status){
      ...
      for(let i = 0; i < seqRows; i++){
        for(let j = 0; j < seqSteps; j++){
          let d = dist(mouseX, mouseY, seqPropertiesObject[`row${i}`][j].x, seqPropertiesObject[`row${i}`][j].y);
          if (d < seqHeight/2) {
            seqPressed(i, j);
          }
        }
...
function seqPressed(row, step) {
  ...
  if(seqStepObject[`row${row}`][step] === 0) { // if the synth is not playing that note at the moment
    seqPropertiesObject[`row${row}`][step].image = seqOn;
    drawSynth();
    seqStepObject[`row${row}`][step] = 1; // change the array to reflect that the note is playing
  }
  else { // if the synth is playing that note at the moment
    seqPropertiesObject[`row${row}`][step].image = seqOff;
    drawSynth();
    seqStepObject[`row${row}`][step] = 0; // change the array to reflect that the note is playing
  }
  console.log(`row${row} step ${step} = ${seqStepObject[`row${row}`][step]}`);
...
function repeat(time) {
  ...
  drawSynth(_step)
  for(let i = 0; i < seqRows; i++) {
    if(seqStepObject[`row${i}`][_step] === 1) {
      seqPlayers[i].start();
    }
  }
  index++;
...
function saveSeq() {
  for(let i = 0; i < seqRows; i++){
    for(let j = 0; j < seqSteps; j++){
      seqSaveSteps[i][j] = seqStepObject[`row${i}`][j];
    }
  }
